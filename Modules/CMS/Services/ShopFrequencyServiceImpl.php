...
class ShopFrequencyServiceImpl implements ShopFrequencyService
{
    ...
    public function getDataDashboardReport(): array
    {
        try {
            ...
            $currentToken = now()->format('Y-m-d') <= $shop->token_expired_date
                ? $shop->current_token_qty : 0;

            $currentTokenSubscribe = now()->format('Y-m-d 23:59:59') <= $shop->token_subscribe_expired_date
            ...
            return [
                ...
                'token_available' => $currentToken + $currentTokenSubscribe,
