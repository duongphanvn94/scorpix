...
use Modules\CMS\Http\Requests\Order\UnsubscribeRequest;
...
class OrderController extends Controller
{
     ...
     */
    public function stripeUnsubscribe(UnsubscribeRequest $request): \Illuminate\Http\RedirectResponse
    {
        $currentSubscriberId = $request->get('currentSubscriberId');

        $resultStore = $this->subscriberHistoryService->unsubscriber($currentSubscriberId);

        if (!$resultStore['result']) {
            return redirect()->back()->with('toast_error', $resultStore['message']);
        }

        return redirect()->route('cms.dashboard')->with('toast_success', $resultStore['message']);
    }
