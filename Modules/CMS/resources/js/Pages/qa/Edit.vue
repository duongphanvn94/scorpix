    ...
    <div class="card">
        <div class="mb-6">
            <div class="flex items-center gap-2 mb-2">
                ...
                <h1 class="text-2xl font-bold text-gray-900">Edit Q&A</h1>
            </div>
            <p class="text-gray-600">Update question and answer</p>
        </div>
        ...
        <form @submit.prevent="submitForm" class="max-w-2xl">
            ...
            <div class="flex gap-3">
                ...
                <Button :loading="submitting" type="submit" label="Update Q&A" class="p-button-primary" />
            </div>
        </form>
    </div>
</template>
...
const submitForm = async () => {
    ...
    try {
        await router.put(`/cms/qa/${props.qa.id}`, {
            ...
            onSuccess: () => {
                toast.add({
                    severity: 'success',
                    summary: 'Success',
                    detail: 'Q&A updated successfully',
                    life: 3000
                })
                router.visit('/cms/qa')
            },
...
const initializeForm = () => {
    if (props.qa) {
        form.question = props.qa.question || ''
        form.answer = props.qa.answer || ''
        form.status = props.qa.status !== undefined ? props.qa.status : true
    }
}
