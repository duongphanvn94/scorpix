<template>
    <div class="card">
        ...
        <DataTable
            ...
            :loading="loading"
            :filters="filters"
            :globalFilterFields="['name', 'admin.email', 'phone_number', 'current_subscription.product.name']"
            showGridlines
        >
            <template #header>
            ...
            <Column field="name" header="Name" style="max-width: 5rem" :sortable="true">
                <template #body="{ data }">
                    <span>{{ data.name || 'N/A' }}</span>
                </template>
            </Column>

            <Column field="email" header="Email" style="max-width: 5rem" :sortable="true">
                <template #body="{ data }">
                    <span>{{ data.admin.email }}</span>
                </template>
            </Column>

            <Column field="phone_number" header="Phone Number" style="max-width: 5rem" :sortable="true">
            </Column>

            <Column field="api_key" header="Api Key" style="max-width: 5rem" :sortable="true">
                <template #body="{ data }">
                  <span>
                    {{
            ...
            <Column field="current_subscription.product_id"
                    filterField="current_subscription.product_id"
                    header="Plan"
                    style="max-width: 5rem"
                    :sortable="true"
                    sortField="current_subscription.product_id"
            >
                <template #body="{ data }">
                    {{ data.current_subscription?.product.name }}
                </template>
            ...
            <Column field="status" header="Status" style="max-width: 5rem" :sortable="true">
                <template #body="{ data }">
                    <Tag
...
function initFilters() {
    filters.value = {
        ...
        'current_subscription.product_id': {operator: FilterOperator.OR, constraints: [{value: null, matchMode: FilterMatchMode.EQUALS}]},
    };
}
